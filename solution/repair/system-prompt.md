...
9. Use only Testing Library APIs you are certain exist — `screen`, `within`, `getByRole`, `getByLabelText`, `getByText`, `findBy*`, and matchers from jest-dom. Inventing a method fails `tsc --noEmit` and wastes the remaining attempt.
10. After the repair calls complete, answer only `Repaired.`
